...
export function MobileNav({ branding }: MobileNavProps) {
    ...
    return (
        <>
            ...
            <div className="md:hidden fixed top-0 left-0 right-0 h-14 bg-[#09090b] border-b border-zinc-800 z-[90] flex items-center justify-between px-4">
                {/* Logo/Brand */}
                <div className="flex items-center gap-3">
                    <div className="h-8 w-8 rounded-md bg-zinc-800 border border-zinc-700 flex items-center justify-center shrink-0 text-white font-bold overflow-hidden">
                        {branding.logo ? (
                            ...
                            branding.name.substring(0, 2)
                        )}
                    </div>
                    <span className="font-semibold text-white text-sm">{branding.name}</span>
                </div>
